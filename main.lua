...
player.CharacterAdded:Connect(function()task.wait(1)refreshCharacter()end)
...
local LOOP_COUNT=1
Tab:CreateInput({Name="🔁 Loop Count",PlaceholderText="1",Callback=function(txt)local n=tonumber(txt)LOOP_COUNT=(n and n>0)and math.floor(n)or 1 end})
...
local function getTimerValue()local n=tonumber(string.match(timerLabel.Text or "","%-?[%d%.]+"))return(n and n>0)and n or 0 end
...
local part=getTargetPart(item)
local prompt=item:FindFirstChildWhichIsA("ProximityPrompt",true)
if not(part and prompt)then return end
if not root or not root.Parent then refreshCharacter()return end
local old=root.CFrame
local offsets={Vector3.new(0,3,0),Vector3.new(2,2,0),Vector3.new(-2,2,0),Vector3.new(0,2,2),Vector3.new(0,2,-2)}
for _,o in ipairs(offsets)do
root.CFrame=part.CFrame+o
task.wait(0.03)
...
else task.wait(2)end end
if STATE=="IDLE"then STATE="COOLDOWN"end
else task.wait(1)end
task.wait(0.1)
end end)

local function instantMix()
for i=1,6 do
remote:FireServer("AlchemyController",false,"mixing",math.random())
task.wait(0.02)
end end
...
while not canCraft(name,data)do task.wait(10)end
while isTimerRunning()do task.wait(2)end

lock()

remote:FireServer("AlchemyController",false,"craft",data)
task.wait(0.2)

instantMix()

repeat task.wait(0.5)until not isTimerRunning()
...
Tab:CreateToggle({Name="🌿 Auto Forage",Callback=function(v)AUTO_FORAGE=v STATE="IDLE"end})
Tab:CreateToggle({Name="🛠 Auto Handcraft",Callback=function(v)AUTO_HAND=v if v then HAND_DONE=0 HAND_TARGET=0 end end})
